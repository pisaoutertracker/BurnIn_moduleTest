...
class IVCurveMeasurement:
    def __init__(self, channels, voltage_steps, delay=5.0, voltage_threshold=0.5, voltage_threshold_time=0.5, voltage_threshold_retries=20, settling_time=0.5, final_voltage=600):
        ...
        self.measurements = {ch: [] for ch in channels}
    ...
    def verify_all_voltages(self, target_voltage, caen_conn=None):
        if not caen_conn:
            raise ValueError("TCP connection required")
            ...
            try:
                ...
                for ch in self.channels:
                    ...
                    if abs(current_voltage - target_voltage) > self.voltage_threshold:
                        ...
                        break
                if all_good:
                    print(f"All voltages reached target {target_voltage} V within threshold, settling...")
    ...
    def measure_curve(self):
        ...
        try:
            ...
            for voltage in self.voltage_steps:
                ...
                # Verify all voltages are within threshold
                if not self.verify_all_voltages(voltage, caen_conn=conn):
                    print(f"Warning: Some channels did not reach target voltage {voltage}V within threshold")
                    continue
                ...
                try:
                    ...
                    for ch in self.channels:
                        ...
                        self.measurements[ch].append(measurement)
                        print(f"Measured for {ch}: V={measurement['Voltage']:.2f} V, I={measurement['Current']:.3f} nA, T={measurement['Temperature']:.1f} C, RH={measurement['Relative Humidity']:.3f} %")
                except Exception as e:
            ...
            # Set final voltage for all channels
            for ch in self.channels:
                try:
    ...
    def save_to_csv_for_module(self, channel, module_name, measurements_list, output_path, run_info):
...
def measure_and_upload(
    ...
    Returns: exit_code
    """
    if len(channels) != len(module_names):
        raise ValueError("Number of channels must match number of module names")
    ...
    # For each module, save file and upload
    for ch, mn in zip(channels, module_names):
        measurements_for_module = iv_curve.measurements[ch]
        if not measurements_for_module:
            print(f"No measurements for {mn}, skipping")
            continue
        
        final_output_file = get_default_filename(ch, mn, scan_type)
        final_output_path = os.path.join(output_dir, final_output_file)
        
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as tmp_file:
            output_path = tmp_file.name
        
        saved_file = iv_curve.save_to_csv_for_module(ch, mn, measurements_for_module, output_path, run_info)
        
        if store_locally:
            os.makedirs(output_dir, exist_ok=True)
            with open(saved_file, 'r') as src, open(final_output_path, 'w') as dst:
                dst.write(src.read())
            print(f"Measurements for {mn} saved to: {final_output_path}")
        
        # Prepare document for this module
        document = {
            "nameLabel": mn,
            ...
            "averageTemperature": sum(m['Temperature'] for m in measurements_for_module) / len(measurements_for_module) if measurements_for_module else -999,
            "averageHumidity": sum(m['Relative Humidity'] for m in measurements_for_module) / len(measurements_for_module) if measurements_for_module else -999,
            "sessionName": session,
            "IVScanId": f"{ch}_{mn}_{scan_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            "data": {
        ...
        if upload:
            ...
            if returncode != 0:
                print(f"WARNING!! Upload to Central DB failed for {mn}")
    
    return 0

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='IV Curve Measurement Script')
    parser.add_argument('--channels', type=str, required=True, help='Channel IDs (comma separated, e.g., HV005,HV006)')
    parser.add_argument('--module-names', type=str, required=True, help='Module names (comma separated, e.g., MOD1,MOD2)')
    ...
    module_names = [mn.strip() for mn in args.module_names.split(',')]
    if len(channels) != len(module_names):
        print("Error: Number of channels must equal number of module names")
        sys.exit(1)
