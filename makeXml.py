...
def makeXmlPyConfig(board, opticalGroups, hybrids, strips, pixels, lpGBTfile='default', edgeSelect='default', outFile="PS_Module_settings_autogenerated.py", Nevents=50):
    config = { 
        "Nevents" : Nevents,
        "boards" : {
            0 : {
    ...
    if lpGBTfile=='default': ## takes default value from xml template
        del config["boards"][0]["opticalGroups"]["X"]["lpGBT"]

    if type(lpGBTfile) is str:
        lpGBTfile = [lpGBTfile]*len(opticalGroups)

    if edgeSelect!='default':
    ...
    for i, o in enumerate(opticalGroups):
        config["boards"][0]["opticalGroups"][o] = dict(config["boards"][0]["opticalGroups"]["X"])
        if config["boards"][0]["opticalGroups"][o]["lpGBT"]:
            config["boards"][0]["opticalGroups"][o]["lpGBT"] = lpGBTfile[i]
