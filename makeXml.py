...
def makeXmlPyConfig(board, slots, hybrids, strips, pixels, lpGBTfile, edgeSelect=1, outFile="PS_Module_settings_autogenerated.py", Nevents=50):
    config = { 
        "Nevents" : Nevents,
        "boards" : {
            0 : {
    ...
    for s in slots:
        config["boards"][0]["opticalGroups"][s] = dict(config["boards"][0]["opticalGroups"]["X"])
...
#    verbose = -1
    ...
    xmlConfig = makeXmlPyConfig("fc7ot2", slots=[2], hybrids=[0], strips= [2,5], pixels= [1,3], lpGBTfile="lpGBT_v1_PS.txt", edgeSelect=None, outFile="PS_Module_settings_autogenerated_test.py", Nevents=50)
    print("\nxmlConfig:")
    from pprint import pprint
    pprint(xmlConfig)
    print()
